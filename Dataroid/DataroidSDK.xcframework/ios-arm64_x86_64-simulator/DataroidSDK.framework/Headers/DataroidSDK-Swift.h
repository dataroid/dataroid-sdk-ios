...
@class DTRProduct;
SWIFT_CLASS_NAMED("AddToCartEventAttributes")
@interface DTRAddToCartEventAttributes : DTRAttributes
- (nonnull instancetype)initWithProduct:(DTRProduct * _Nonnull)product OBJC_DESIGNATED_INITIALIZER;
- (DTRAddToCartEventAttributes * _Nonnull)addValue:(NSDecimal)value;
- (DTRAddToCartEventAttributes * _Nonnull)addTotalCartValue:(NSDecimal)value;
...
- (void)collectAddToCart:(DTRAddToCartEventAttributes * _Nonnull)attributes;
...
@property (nonatomic, readonly, strong) id <SurveyClientProtocol> _Nonnull survey;
...
@property (nonatomic, strong) SurveyConfig * _Nonnull survey;
...
/// Event name for holdout in-app exposure, shouldn’t be used manually
SWIFT_CLASS_PROPERTY(@property (nonatomic, class, readonly, copy) NSString * _Nonnull inAppExposureHoldout;)
+ (NSString * _Nonnull)inAppExposureHoldout SWIFT_WARN_UNUSED_RESULT;
/// Event name for holdout goal achieved, shouldn’t be used manually
SWIFT_CLASS_PROPERTY(@property (nonatomic, class, readonly, copy) NSString * _Nonnull goalAchievedHoldout;)
+ (NSString * _Nonnull)goalAchievedHoldout SWIFT_WARN_UNUSED_RESULT;
...
/// Event name for survey-list-fetched, shouldn’t be used manually
SWIFT_CLASS_PROPERTY(@property (nonatomic, class, readonly, copy) NSString * _Nonnull surveysFetched;)
+ (NSString * _Nonnull)surveysFetched SWIFT_WARN_UNUSED_RESULT;
/// Event name for survey container display, shouldn’t be used manually
SWIFT_CLASS_PROPERTY(@property (nonatomic, class, readonly, copy) NSString * _Nonnull surveyDisplayed;)
+ (NSString * _Nonnull)surveyDisplayed SWIFT_WARN_UNUSED_RESULT;
/// Event name for survey end (per-step interactions + endReason),
/// shouldn’t be used manually
SWIFT_CLASS_PROPERTY(@property (nonatomic, class, readonly, copy) NSString * _Nonnull surveyEnded;)
+ (NSString * _Nonnull)surveyEnded SWIFT_WARN_UNUSED_RESULT;
@end
...
/// Screen name associated with this APM event.
/// Set this when the request was initiated on a different screen than the one visible at
/// response-completion time (e.g. a page transition occurred mid-flight). <code>viewId</code> is derived
/// from this value automatically. If left <code>nil</code>, the SDK falls back to the last tracked screen.
@property (nonatomic, copy) NSString * _Nullable viewLabel;
/// View controller class name associated with this APM event (e.g. <code>"HomeViewController"</code>).
/// Optional companion to <code>viewLabel</code>. If left <code>nil</code>, the SDK falls back to the last tracked screen.
@property (nonatomic, copy) NSString * _Nullable viewClass;
...
enum DTRInAppDisplayType : NSInteger;
/// Snapshot describing an in-app message right before it is presented.
/// Delivered to the host app through <code>InAppMessageListener/willShowInApp(_:)</code>
/// so the host can react (e.g. pause media, adjust UI) just before display.
/// The populated fields depend on <code>type</code>:
/// <ul>
///   <li>
///     <code>InAppDisplayType/native</code>: <code>inappId</code>, <code>triggerEvent</code>, <code>dynamicAttributes</code>.
///   </li>
///   <li>
///     <code>InAppDisplayType/custom</code>: <code>inappId</code>, <code>triggerEvent</code>, <code>dynamicAttributes</code>,
///     <code>location</code>, <code>horizontalLocation</code>.
///   </li>
/// </ul>
SWIFT_CLASS_NAMED("InAppDisplayInfo")
@interface DTRInAppDisplayInfo : NSObject
/// Whether the message is native or custom (HTML).
@property (nonatomic, readonly) enum DTRInAppDisplayType type;
/// Backend identifier of the in-app message.
@property (nonatomic, readonly, copy) NSString * _Nullable inappId;
/// Name of the event that triggered this in-app message.
@property (nonatomic, readonly, copy) NSString * _Nullable triggerEvent;
/// Dynamic attributes injected by the backend for this message.
@property (nonatomic, readonly, copy) NSDictionary<NSString *, id> * _Nullable dynamicAttributes;
/// Vertical placement (<code>TOP</code>, <code>BOTTOM</code>, <code>FULLSCREEN</code>). Custom messages only.
@property (nonatomic, readonly, copy) NSString * _Nullable location;
/// Horizontal alignment (<code>LEFT</code>, <code>CENTER</code>, <code>RIGHT</code>). Custom messages only.
@property (nonatomic, readonly, copy) NSString * _Nullable horizontalLocation;
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
+ (nonnull instancetype)new SWIFT_UNAVAILABLE_MSG("-init is unavailable");
@end

/// Kind of in-app message that is about to be presented.
typedef SWIFT_ENUM_NAMED(NSInteger, DTRInAppDisplayType, "InAppDisplayType", open) {
/// A native <code>UIAlertController</code>-based in-app message.
  DTRInAppDisplayTypeNative = 0,
/// An HTML (<code>WKWebView</code>-based) custom in-app message.
  DTRInAppDisplayTypeCustom = 1,
};
...
typedef SWIFT_ENUM_NAMED(NSInteger, DTRInAppMessageDeliveryType, "InAppMessageDeliveryType", open) {
  DTRInAppMessageDeliveryTypeStandard = 0,
  DTRInAppMessageDeliveryTypeHoldout = 1,
};
...
/// Delegate method that will be invoked right before an in-app message is
/// presented on screen by the SDK.
/// \param inappDisplayInfo Snapshot describing the in-app message
/// (type and the relevant native/custom fields) about to be shown.
///
- (void)willShowInApp:(DTRInAppDisplayInfo * _Nonnull)inappDisplayInfo;
@end
...
@protocol DTRSurveyListener;
/// Public surface for the Survey module.
SWIFT_PROTOCOL("_TtP11DataroidSDK20SurveyClientProtocol_")
@protocol SurveyClientProtocol
/// Listener notified right before a survey is displayed.
@property (nonatomic, strong) id <DTRSurveyListener> _Nullable surveyListenerDelegate;
@end

/// Host-app facing toggle for the Survey module.
/// Companion to the remote-config flag (<code>features.survey</code>): the survey pipeline
/// only runs when <em>both</em> the remote flag and this client-side switch are on.
SWIFT_CLASS("_TtC11DataroidSDK12SurveyConfig")
@interface SurveyConfig : NSObject
/// Master switch for surveys on the host side. Defaults to <code>true</code> so apps
/// that opt in via remote config don’t need additional setup.
@property (nonatomic) BOOL enabled;
- (nonnull instancetype)init OBJC_DESIGNATED_INITIALIZER;
@end

/// Snapshot describing a survey right before it is presented.
/// Delivered to the host app through <code>SurveyListener/willShowSurvey(_:)</code> so
/// the host can react (e.g. pause media, adjust UI) just before display.
SWIFT_CLASS_NAMED("SurveyDisplayInfo")
@interface DTRSurveyDisplayInfo : NSObject
/// Backend identifier of the survey.
@property (nonatomic, readonly, copy) NSString * _Nullable surveyId;
/// Name of the event that triggered this survey.
@property (nonatomic, readonly, copy) NSString * _Nullable triggerEvent;
/// Dynamic attributes injected by the backend for this survey.
@property (nonatomic, readonly, copy) NSDictionary<NSString *, id> * _Nullable dynamicAttributes;
/// Vertical placement (<code>TOP</code>, <code>BOTTOM</code>, <code>FULLSCREEN</code>).
@property (nonatomic, readonly, copy) NSString * _Nullable location;
...
/// Interface for the objects that listen to survey display events.
SWIFT_PROTOCOL_NAMED("SurveyListener")
@protocol DTRSurveyListener
@optional
/// Delegate method that will be invoked right before a survey is presented
/// on screen by the SDK.
/// \param surveyDisplayInfo Snapshot describing the survey about to
/// be shown.
///
- (void)willShowSurvey:(DTRSurveyDisplayInfo * _Nonnull)surveyDisplayInfo;
@end
